...
script="./inject_init_bc.sh -s ${server_ip}"
...
if sudo docker ps --format "" | grep -q "${container_image}"; then
        ...
        echo -e "${LIGHT_GREEN}${HOSTNAME}:${NC} ${YELLOW}Container with image ${server_image} is already running${NC}"
else
    output=$(sudo docker run --gpus all -d -p 4000:4000 --network host ${container_image} ${script} 2>&1)
